"""
T4.4 (B) — Ablation for the "find SECOND occurrence of max" task.

The task plants two identical max tokens at random positions p1 < p2 per
row and asks the model to predict p2.  Because the two max tokens are
content-identical, the model MUST use positional information to
distinguish them.  This is the ablation case where  use_pe=False
should fail dramatically.

Run
---
    python experiments/ablation_second_max.py
"""
...
from data import gen_second_max_batch
...
BASE_CONFIG = {
    "vocab_size":   10,
    "n":            20,           # back to 20 — task is harder, n=100 unnecessary
    "d_model":      32,
    "num_heads":    4,
    "num_layers":   2,
    "dropout":      0.0,
...
VARIANTS = [
    ("baseline",  {}),
    ("no_pe",     {"use_pe": False}),         # expected to FAIL
...
# --------------------------------------------------------------------------- #
# Eval + train  (identical structure to ablation.py, only the data fn swapped) #
# --------------------------------------------------------------------------- #
...
def evaluate(model, n_batches, B, n, vocab_size, device):
    ...
    for _ in range(n_batches):
        x, y = gen_second_max_batch(B, n, vocab_size, device=device)
        logits = model(x)
...
def train_one_run(cfg, device):
    ...
    for step in range(1, cfg["num_steps"] + 1):
        model.train()
        x, y = gen_second_max_batch(cfg["batch_size"], cfg["n"],
...
def main():
    ...
    print(f"task    = find SECOND occurrence of max (two identical max tokens per row)")
    ...
    print()
    print("=" * 80)
    print("ABLATION RESULTS  (task: find second occurrence of max)")
    print("=" * 80)
